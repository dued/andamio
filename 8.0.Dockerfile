...
ENV DB_FILTER=.* \
    DEPTH_DEFAULT=1 \
    DEPTH_MERGE=100 \
    EMAIL=https://hub.docker.com/r/dued/odoo \
    GEOIP_ACCOUNT_ID="" \
    GEOIP_LICENSE_KEY="" \
...
# Otros requisitos y recomendaciones para ejecutar Odoo
...
# Caso especial para obtener el último cliente PostgreSQL
...
# Caso especial para obtener las últimas Less y PhantomJS PhantomJS
...
COPY lib/andamiolib /usr/local/lib/python2.7/dist-packages/andamiolib
RUN ln -s /usr/local/lib/python2.7/dist-packages/andamiolib \
...
RUN mkdir -p auto/addons auto/geoip custom/src/private \
    ...
    && chmod -R a+rX /usr/local/lib/python2.7/dist-packages/andamiolib \
...
# Dependencias Andamiooo-QA en un virtualenv separado
COPY qa /qa
...
# Ejecutar el script de instalación por versión Odoo
# Esto es al final para beneficiarse de la caché en el momento de la construcciónomento de la construcción
...
LABEL org.label-schema.schema-version="$VERSION" \
      ...
      org.label-schema.vcs-url="https://github.com/dued/andamio"

# Versión incorporada, con toda la magia
FROM base AS onbuild

# Disparadores de subimagen
ONBUILD USER root
...
# Habilite la configuración de uid's personalizados para el usuario de odoo durante la construcción de andamios
ONBUILD ARG UID=1000
ONBUILD ARG GID=1000

# Habilitar usuario de Odoo y filestore
...
ONBUILD USER odoo
# HACK caso especial para Werkzeug
