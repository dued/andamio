...
# Caso especial para obtener las últimas Less y PhantomJS
...
COPY lib/andamiolib /usr/local/lib/python3.7/site-packages/andamiolib
RUN ln -s /usr/local/lib/python3.7/site-packages/andamiolib \
...
RUN mkdir -p auto/addons auto/geoip custom/src/private \
    ...
    && chmod -R a+rX /usr/local/lib/python3.7/site-packages/andamiolib \
...
# Andamio-QA dependencies in a separate virtualenv
COPY qa /qa
...
# Esto es al final para beneficiarse de la caché en el momento de la construcción
