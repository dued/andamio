...
# establece el host, el puerto, el usuario y la contraseña de la base de datos
# de Postgres de acuerdo con el entorno y los pasa como argumentos al proceso de
# odoo si no están presentes en el archivo de configuración
...
if [ -z "$EXTRA_ADDONS_PATHS" ]; then
    echo "La variable \$EXTRA_ADDONS_PATHS Está vacía, usando addons_path predeterminado"
else
        ...
        if [[ "$1" == "scaffold" ]] ; then
            ...
            if [[ "$UPGRADE_ODOO" -eq "1" ]] ; then
                ...
                for db in ${ODOO_DB_LIST}; do
                    ...
                    echo "La base datos ${db} Ha sido actualizado"
                done
            fi
            exec odoo "$@" "${DB_ARGS[@]}"
        fi
